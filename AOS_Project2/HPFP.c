...
average_stats highestPriorityFirstP(linked_list * processes)
{
  ...
  while(t<100 || Prs_Scheduled!=NULL )
  {
    ...
    {
      if(Prs_Scheduled->proc->priority == 1) enqueue(PQueue1,Prs_Scheduled);
      if(Prs_Scheduled->proc->priority == 2) enqueue(PQueue2,Prs_Scheduled);
      if(Prs_Scheduled->proc->priority == 3) enqueue(PQueue3,Prs_Scheduled);
      if(Prs_Scheduled->proc->priority == 4) enqueue(PQueue3,Prs_Scheduled);
    ...
    //check for incoming new process and enqueue it in the queue
        ...
        if(new_process->priority == 4) enqueue(PQueue3,createProcessStat(new_process));
				sort(PQueue1,ComparePriority);
        sort(PQueue2,ComparePriority);
        sort(PQueue3,ComparePriority);
        sort(PQueue4,ComparePriority);
